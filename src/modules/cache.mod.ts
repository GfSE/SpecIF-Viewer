...
/*	var autoLoadId,				// max 1 autoLoad chain
	...
	/*	autoLoadId = undefined;  // stop any autoLoad chain
		...
		self.create = (p, opts:object ): JQueryPromise<yoid> => {
...
function Project() {
	...
	self.init = ():void =>{
		// initialize/clear all variables:
	//	self.data = {} as SpecIF;  --> doesn't work (yet)
		self.data = {
			id: '',
			title: '',
			description: '',
	...
	self.create = (newD, opts ): JQueryPromise<void> => {
...
function classifyProps( el:Resource|Statement, prj? ) {
...
//	console.debug( 'classifyProps 2', simpleClone(cP) );
	...
	function normalizeProps(el: Resource | Statement, dta): Array<Property> {
