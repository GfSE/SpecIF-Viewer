...
//				console.debug('cache',ctg,item);
	...
	del(ctg: string, item: Item): boolean | undefined {
		if (!item) return;
...
/*	myRole = i18n.LblRoleProjectAdmin;
	...
	//	this.exp = true;
		...
		this.types = [
			new CElement('dataType', this.equalDT, this.compatibleDT, this.substituteDT.bind(this)),
			new CElement('propertyClass', this.equalPC, this.compatiblePC.bind(this), this.substitutePC.bind(this)),
			new CElement('resourceClass', this.equalRC.bind(this), this.compatibleRC.bind(this), this.substituteRC.bind(this)),
			new CElement('statementClass', this.equalSC, this.compatibleSC.bind(this), this.substituteSC.bind(this))
		];
	};
	isLoaded(): boolean {
...
//		console.debug('app.cache.selectedProject.data.create',newD);
		...
		if (newD.isValid()) {
			...
			pend = standardTypes.iterateLists(
				(key,value) => {
					this.createContent(key, newD[value])
			...
			sDO.reject({ status: 995, statusText: i18n.lookup('MsgImportFailed', newD.title) });
		};
		return sDO;
...
/*	update(newD: SpecIF, opts: any) {
		var uDO = $.Deferred();
		newD = new CSpecIF(newD); // transform to internal data structure
		uDO.resolve();
		return uDO;
	} */
	adopt(newD: SpecIF, opts?: any) {
...
//			console.debug('#3',simpleClone(dta),simpleClone(newD));
			...
			uDO.reject({ status: 995, statusText: i18n.lookup('MsgImportFailed', newD.title) });
		};
		return uDO;
				...
				this.readContent('statementClass', 'all')
					...
					.then(
						(sL: Statement[]) => {
							...
							let showsL = sL.filter((s) => { return staClassTitleOf(s) == CONFIG.staClassShows && isReferencedByHierarchy(Lib.itemIdOf(s.subject)) });
							// filter all statements involving res as subject or object:
							resolve(
								sL.filter(
									(s) => {
										let sC: StatementClass = itemById(sCL, s['class'] as string);
										return (res.id == Lib.itemIdOf(s.subject) || res.id == Lib.itemIdOf(s.object))
											...
											&& (
												...
												&& isReferencedByHierarchy(Lib.itemIdOf(s.subject))
												&& isReferencedByHierarchy(Lib.itemIdOf(s.object))
...
//			console.debug('eqSCL',rL,nL);
	...
	private equalKey(r: KeyObject, n: KeyObject): boolean {
		...
		return Lib.itemIdOf(r) == Lib.itemIdOf(n)
...
//					console.debug('substituteR 3a',nP,pT,rP,Lib.hasContent(valByTitle( r, pT, this.data )));
		...
		prj.statements.forEach((st: Statement) => {
			if (this.equalKey(st.object, n)) { if (st.object.id) { st.object.id = Lib.itemIdOf(r) } else { st.object = Lib.itemIdOf(r) } };
			if (this.equalKey(st.subject, n)) { if (st.subject.id) { st.subject.id = Lib.itemIdOf(r) } else { st.subject = Lib.itemIdOf(r) } }
...
Lib.itemIdOf = ( key:KeyObject|string ):string =>{
...
function collectResourcesByHierarchy(prj: SpecIF, H?: SpecifNode[] ):Resource[] {
	...
	var rL:Resource[] = [];
	Lib.iterateNodes( H, (nd)=>{ Lib.cacheE( rL, itemById(prj.resources,Lib.itemIdOf(nd.resource)) ); return true } );
	return rL;
}
