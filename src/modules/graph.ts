...
class CGraphOptions {
	// @ts-ignore - opts.canvas will be defined in the constructor
	canvas: string;
	index = 0;
	...
	constructor(opts: object) {
		// @ts-ignore - opts.canvas is required
...
/*  private statementById(id) {
		for(var i = this.statements.length-1; i>-1; i--)
			if (this.statements[i].id === id) return this.statements[i];
		return // undefined
	} */
    ...
    */
	...
	show(opts: CGraphOptions): void {
		...
		if (!opts.index || opts.index > this.resources.length - 1) opts.index = 0;
		...
		let relProp = countRelationTypesAndEdges(relations),
			idx = pushMainNode(this.resources[opts.index]);  // returns always 1
		...
		network.getConnectedNodes("0").forEach( (connectedNode: any) => {
		...
		function closeCluster(node: number, network: any) {
			if (node === 0) {
				network.getConnectedNodes("0").forEach( (connectedNode: number) => {
					closeCluster(connectedNode, network)
				})
			};
			let options = {
