...
function transformReqif2Specif(reqIfDocument) {               
    ...
    specIfObject.dataTypes = extractSpecifDatatypesFromXmlDoc(xmlDoc.getElementsByTagName("DATATYPES"));
    specIfObject.propertyClasses = extractSpecifPropertyClassesFromXmlDoc(xmlDoc.getElementsByTagName("SPEC-TYPES"));
    specIfObject.resourceClasses = extractSpecifResourceClassesFromXmlDoc(xmlDoc.getElementsByTagName("SPEC-TYPES"));
    specIfObject.statementClasses = extractSpecifStatementClassesFromXmlDoc(xmlDoc.getElementsByTagName("SPEC-TYPES"));
	specIfObject.resources = extractSpecifResourcesFromXmlDoc(xmlDoc.getElementsByTagName("SPEC-OBJECTS"))
								// add the hierarchy roots to the resouce list:
								.concat(extractSpecifResourcesFromXmlDoc(xmlDoc.getElementsByTagName("SPECIFICATIONS")));
    specIfObject.statements = xmlDoc.getElementsByTagName("SPEC-RELATIONS")[0] ? extractSpecifStatementsFromXmlDoc(xmlDoc.getElementsByTagName("SPEC-RELATIONS")) : [];
    specIfObject.hierarchies = xmlDoc.getElementsByTagName("SPECIFICATIONS")[0] ? extractSpecifHierarchiesFromXmlDoc(xmlDoc.getElementsByTagName("SPECIFICATIONS")) : [];
...
function extractSpecifDatatypesFromXmlDoc(XmlDocDatatypes) {
	return XmlDocDatatypes.length<1? [] : Array.from(XmlDocDatatypes[0].children, extractSpecifDatatype );

	function extractSpecifDatatype(datatype) {
		let specifDatatype = {};
...
function extractSpecifPropertyClassesFromXmlDoc(XmlSpecTypeDocument) {
...
function extractSpecifResourceClassesFromXmlDoc(XmlSpecTypeDocument) {
    ...
    Array.from(XmlSpecTypeDocument[0].children,
		classDocument => {
			if( isResourceClass(classDocument) )
				specifResourceClassesArray.push(extractSpecIfElementClass(classDocument));
		}
	);
...
function extractSpecifStatementClassesFromXmlDoc(XmlSpecTypeDocument) {
    if (XmlSpecTypeDocument.length<1) return [];
    ...
    Array.from(XmlSpecTypeDocument[0].children,
		classDocument => {
			if( isStatementClass(classDocument) )
				specifStatementClassesArray.push(extractSpecIfElementClass(classDocument));
		}
	);
...
function extractSpecIfElementClass(classDocument) {
...
function extractSpecifResourcesFromXmlDoc(XmlDocResources) {
	return XmlDocResources.length<1? [] : Array.from(XmlDocResources[0].children,extractSpecIfResource);

	function extractSpecIfResource(resourceDocument) {
		let specifResource = {};
...
function extractSpecifStatementsFromXmlDoc(XmlDocStatements) {
	return XmlDocStatements.length<1? [] : Array.from(XmlDocStatements[0].children,extractSpecIfStatement);

	function extractSpecIfStatement(statementDocument) {
		let specifStatement = {};
...
function extractSpecifHierarchiesFromXmlDoc(XmlDocSpecifications) {
