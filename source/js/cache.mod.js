...
//		console.debug('createProcessesFolder',dta.hierarchies);
		iterateNodes( dta.hierarchies, (nd)=>{
...
//						console.debug('glossary',glossary);
		...
		function NodeList(resL) {
			...
			self.data.resourceClasses.forEach( (rC)=>{
	...
	//			console.debug( "publish", opts );
				...
				let data = specif.toExt( self.data, opts ), 
					...
					//	hasContent: hasContent,
						...
						done: ()=>{ app.cache.selectedProject.data.exporting=false; resolve() },
						fail: (xhr)=>{ app.cache.selectedProject.data.exporting=false; reject(xhr) }
					},
					pend=0;
